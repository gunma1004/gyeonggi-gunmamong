...
function showMainPage() {
  ...
  const mainImg = "https://www.gyeonggi-hometai.shop/images/logo.png";
  const mainUrl = "https://www.gyeonggi-hometai.shop/";
...
function showDetailPage(area, guName, dongName) {
  ...
  if (guName && guName !== "시 관할" && guName !== "권역") {
    pageTitle = `${locationTitle} ${guName} 홈타이 · 출장 방문마사지 안내`;
    pageDesc = `${locationTitle} ${guName} 지역 검증된 홈타이, 출장 타이 마사지, 방문 아로마 마사지 제휴업체 및 후불제 예약 안내.`;
  }
  
  if (dongName) {
    pageTitle = `${locationTitle} ${dongName} 홈타이 · 출장 방문마사지 안내`;
    pageDesc = `${locationTitle} ${dongName} 지역 맞춤형 출장 타이 마사지, 방문마사지 제휴업체 정보 및 실시간 후불제 예약 상담.`;
  }
